...
AGE_F2Character::AGE_F2Character()
{
...
void AGE_F2Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("MoveForward", this, &AGE_F2Character::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &AGE_F2Character::MoveRight);

	// We have 2 versions of the rotation bindings to handle different kinds of devices differently
	// "turn" handles devices that provide an absolute delta, such as a mouse.
	// "turnrate" is for devices that we choose to treat as a rate of change, such as an analog joystick
	PlayerInputComponent->BindAxis("Turn", this, &APawn::AddControllerYawInput);
	PlayerInputComponent->BindAxis("TurnRate", this, &AGE_F2Character::TurnAtRate);
	PlayerInputComponent->BindAxis("LookUp", this, &APawn::AddControllerPitchInput);
	PlayerInputComponent->BindAxis("LookUpRate", this, &AGE_F2Character::LookUpAtRate);

	// handle touch devices
	PlayerInputComponent->BindTouch(IE_Pressed, this, &AGE_F2Character::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &AGE_F2Character::TouchStopped);

	// VR headset functionality
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &AGE_F2Character::OnResetVR);
}

void AGE_F2Character::OnResetVR()
{
	// If GE_F2 is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in GE_F2.Build.cs is not automatically propagated
...
void AGE_F2Character::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void AGE_F2Character::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}

void AGE_F2Character::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void AGE_F2Character::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

void AGE_F2Character::MoveForward(float Value)
{
...
void AGE_F2Character::MoveRight(float Value)
{
	if ( (Controller != nullptr) && (Value != 0.0f) )
	{
...
void AGE_F2Character::BeginPlay()
{
	Super::BeginPlay();
...
void AGE_F2Character::Tick(float DeltaTime)
{
	if (GetVelocity().Size() == 0)
	{
		...
		FRotator LeftFootRot = GetActorRotation();
		FHitResult LeftTraceHit;
